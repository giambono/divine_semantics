...
def create_folder_structure():
    ...
    folders = ["experiments/embeddings", "experiments/models", "experiments/results", "experiments/weights"]
...
def get_embeddings_filename(model_name, types_hash=None):
    ...
    if types_hash is None:
        return os.path.join("experiments", "embeddings", f"{model_name}", "embeddings.parquet")
    return os.path.join("experiments", "embeddings", f"{model_name}_{types_hash}", "embeddings.parquet")
...
def get_results_filename(model_name, weights_name, weights_hash=None):
    ...
    if weights_hash is None:
        return os.path.join("experiments", "results", f"{model_name}_{weights_name}", "embeddings.parquet")
    return os.path.join("experiments", "results", f"{model_name}_{weights_name}_{weights_hash}", "embeddings.parquet")
